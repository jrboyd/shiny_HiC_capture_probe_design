...
probes_dt = rbind(fread("data_round1/Group_A_Probes.txt"), fread("data_round1/Group_B_Probes.txt"))
probes_gr = GRanges(probes_dt$Coordinates)
probes_gr$probe_id = probes_dt$ProbeID

frags_gr = rtracklayer::import.bed("data_round1_hg19mixup/DE_hg38_150bp5000bp_25%65%_1000downandupstream_1perTSS_0ext_2probesPer.bed")
strand(frags_gr) = "*"

ref_bfc = BiocFileCache(cache = ".cache_ref")
ref_gr = readRDS(bfcrpath(ref_bfc, "hg38,DE"))
...
pdf("probe_filtering_tracks1.pdf")
...
write.table(ref_df, "Ref_with_fragments_and_probes.bed", sep = "\t", row.names = FALSE, quote = FALSE)
...
rtracklayer::export.bed(final_probes_gr, "Selected_probes.bed")        
